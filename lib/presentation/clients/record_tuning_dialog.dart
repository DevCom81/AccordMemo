...
import 'record_tuning_error_messages.dart';

Future<bool> showRecordTuningDialog({
  ...
  final saved = await showDialog<bool>(
    ...
    builder: (dialogContext) {
      return RecordTuningDialog(
        today: today,
        onSubmit: ({required CalendarDate tuningDate, String? notes}) {
          return ref.read(recordTuningProvider).execute(
            pianoId: pianoId,
...
class RecordTuningDialog extends StatefulWidget {
  const RecordTuningDialog({
    super.key,
    required this.today,
  ...
  @override
  State<RecordTuningDialog> createState() => _RecordTuningDialogState();
}

class _RecordTuningDialogState extends State<RecordTuningDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      setState(() {
        _saving = false;
        _error = recordTuningMessage(error);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      scrollable: true,
      title: const Text(clientsRecordTuningTitle),
      content: SizedBox(
        width: 420,
        child: Column(
      ...
      actions: [
        ...
        FilledButton(
          ...
          child: const Text(clientsRecordTuningSave),
        ),
      ],
    );
  }
